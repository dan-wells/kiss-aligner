...
allow_repetitions=false  # Update reference text if 1-2 repeated words appear in hypothesis, e.g. for dysfluencies.
ctm_edits_nsw=  # List of non-scored words to pass to steps/cleanup/internal/modify_ctm_edits.py, which can be included in reference text updates
...
if [ $stage -le 11 ]; then
  ...
  [ -z "$ctm_edits_nsw" ] && ctm_edits_nsw=$dir/non_scored_words.txt
  $cmd $dir/log/modify_ctm_edits.log \
    steps/cleanup/internal/modify_ctm_edits.py --verbose=3 \
    --allow-repetitions=$allow_repetitions $ctm_edits_nsw \
